...
- 保存笔记并在内置编辑器中继续修改
- 支持公开只读分享链接
- 支持登录用户创建自己的 API Key，并通过后端 `/api/v1` 外部生成接口调用笔记生成
...
- `GET/POST/DELETE /api/api-keys` 管理登录用户自己的外部调用 API Key
- `POST /api/v1/generate` 与 `POST /api/v1/generate_from_upload` 提供 API Key 保护的外部调用入口
