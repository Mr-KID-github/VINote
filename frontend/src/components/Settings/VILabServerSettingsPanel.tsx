...
import { Box, CheckCircle2, Eye, Globe2, Monitor, RefreshCw, Wifi } from 'lucide-react'
import { useVILabServerStore, type VILabServerMode, type VILabServerModel } from '../../stores/vilabServerStore'
...
function runtimeStatus(model: VILabServerModel) {
  return String(model.runtimeStatus || model.runtime_status || model.status || 'available')
}
...
export function VILabServerSettingsPanel() {
  const { connection, models, loading, testing, saving, error, loadConnection, saveConnection, testConnection, loadModels, clearError } = useVILabServerStore()
  ...
  const speechModels = useMemo(() => models.filter((model) => modelType(model) === 'asr'), [models])
  const languageModels = useMemo(() => models.filter((model) => modelType(model) === 'llm'), [models])
      ...
      <section className="rounded-3xl border border-gray-200 bg-white p-6 shadow-sm dark:border-gray-800 dark:bg-[#1b1b1b]">
        ...
        {!connected ? (
          ...
          <div className="space-y-6">
            ...
            {!speechModels.length && !languageModels.length && <p className="rounded-2xl border border-gray-200 p-8 text-center text-sm text-gray-500 dark:border-gray-800">No models returned by this VILab Server.</p>}
          </div>
        )}
      </section>
    </div>
